...
export async function POST(req: NextRequest) {
  ...
  const { prompt, geminiApiKey } = body as {
    prompt: string;
    geminiApiKey?: string;
  };
  ...
  (async () => {
    try {
      ...
      const design = await agentDesignSystem(analysis, apiKey);
      ...
      const content = await agentContentWriter(analysis, design, apiKey);

      await send({
        type: "agent_chat",
        agent: "Content Writer",
        message: `Hero: "${content.heroTitle}" — "${content.heroSubtitle}"`,
      });
      await send({
        type: "agent_chat",
        agent: "Content Writer",
        message: `Generated ${content.sections.length} sections, ${content.items.length} items, ${content.testimonials.length} testimonials`,
      });
      await send({
        type: "agent_done",
        agent: "Content Writer",
      ...
      await send({
        ...
        message: `Generating multi-file Next.js project with ${design.style} design...`,
      });

      const rawCode = await agentCodeGenerator(analysis, design, content, apiKey);

      // Parse into individual files
      const files = parseFilesFromResponse(rawCode);
      ...
      await send({
        type: "agent_done",
        agent: "Code Generator",
        message: `${files.length} files generated and written to generated-projects/${projectId}`,
        projectId,
      });
      ...
      const validation = await agentValidator(files, apiKey);
